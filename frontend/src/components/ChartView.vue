...
function updateChartLabel() {
    ...
    myLineChart.value.options.scales.y.suggestedMin = getMinValue(...curveStore.filteredData);
...
function removeLine() {
    ...
    myLineChart.value.data.datasets.splice(curveStore.removedLineIndex, 1);
    myLineChart.value.update();
}
...
function toggleLine() {
    ...
    let meta = myLineChart.value.getDatasetMeta(curveStore.toggleLineIndex);
...
function createChart() {
    ...
    myLineChart.value = new Chart(ctx, {
        type: "line",
        data: {
            datasets: [...curveStore.filteredData].map((dataSet, index) => ({
                label: curveStore.getSelectedDays[index],
                data: dataSet, // Y-axis data
                borderColor: curveStore.lineColors[index], // Different color for each line
                borderWidth: 2,
                pointRadius: 5,
                pointHoverRadius: 7,
                backgroundColor: hexToRGBA(curveStore.lineColors[index], 0.3),
                fill: false,
            })),
        },
        options: {
            ...
            scales: {
                ...
                y: {
                    beginAtZero: false,
                    suggestedMin: getMinValue(curveStore.allData),
                    suggestedMax: getMaxValue(curveStore.allData),
                    ticks: {
                        font: {
                            size: adjustFontSize(),
                        }
                    ...
                    title: {
                        ...
                        text: curveStore.language === "arm" ? "Արժեքը %-ով" : "Value in %",
                        font: {
                            weight: "bold",
                        },
                    },
