...
const weekDays = computed(() => {
    return ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map((_, i) =>
        format(new Date(2022, 0, i + 3), 'eee', { locale: indexStore.language === "arm" ? hy : enUS }).toUpperCase()
    );
});

const months = Array.from({ length: 12 }, (_, i) =>
    format(new Date(0, i), 'MMM', { locale: indexStore.language === "arm" ? hy : enUS })
);
...
const getCellClass = (day, index) => {
    ...
    const startDate = parseDate(indexStore.startDate);
    const endDate = parseDate(indexStore.endDate);

    if (
        indexStore.startDate === `${day}/${selectedMonth.value}/${selectedYear.value}` ||
        indexStore.endDate === `${(day)}/${selectedMonth.value}/${selectedYear.value}`
    ) {
        return 'cell--active';
    }
...
const updateCalendar = () => {
    if (selectedYear.value < 2013) selectedYear.value = 2013;
...
<template>
    <div class="calendar">
        <div class="calendar__head">
            ...
            <div class="calendar__head_date">
                <select class="calendar__head_date_month" v-model="selectedMonth" @change="updateCalendar">
                    <option v-for="(month, index) in months" :key="index" :value="index + 1">{{ month }}</option>
                </select>
                <input type="number" class="calendar__head_date_year" v-model="selectedYear" min="2013"
        ...
        <div class="calendar__body">
            ...
            <div v-for="(day, index) in daysArray" :key="`day${index}`" class="cell" :class="getCellClass(day, index)"
                @click="indexStore.changeDates(day, selectedMonth, selectedYear)">
                {{ day }}
            </div>
        </div>
    </div>
...
.calendar__head_date_month {
    font-size: 22px;
    border: none;
    cursor: pointer;
}

#calendar__head_date_month>option {
    font-size: 20px;
    padding: 10px;
}
