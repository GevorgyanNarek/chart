...
const weekDays = computed(() => {
    return ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map((_, i) =>
        format(new Date(2022, 0, i + 3), 'eee', { locale: yieldStore.language === "arm" ? hy : enUS }).toUpperCase()
    );
});

const months = Array.from({ length: 12 }, (_, i) =>
    format(new Date(0, i), 'MMM', { locale: yieldStore.language === "arm" ? hy : enUS })
);
    ...
    <div class="calendar">
        <div class="calendar__head">
            ...
            <div class="calendar__head_date">
                ...
                <input type="number" class="calendar__head_date_year" v-model="selectedYear" min="2016"
        ...
        <div class="calendar__body">
            ...
            <div v-for="(day, index) in daysArray" :key="`day${index}`" class="cell" :class="getCellClass(day, index)"
                @click="yieldStore.toggleDay(day, months[selectedMonth - 1], selectedYear)">
                {{ day }}
            </div>
        </div>
        <div class="calendar__message" v-if="yieldStore.errorMsg">
            {{ yieldStore.language === "arm" ?
                "Առկա չէ տվյալներ " + yieldStore.errorMsg :
                "Data unavailable on " + yieldStore.errorMsg
            }}
        </div>
    </div>
</template>
